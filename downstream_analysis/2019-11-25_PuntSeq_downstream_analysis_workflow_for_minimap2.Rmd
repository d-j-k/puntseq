...
### Pull single downsampling datasets (and three replicates) for downstream analysis
# This section is only nessesary if a new subsampling is performed.
```{python, eval=FALSE}
import pandas as pd
import io
import os
...
# load all rarefied datasets of April, June and August (choose family or genus level)

## if genus:
#url = 'https://raw.githubusercontent.com/d-j-k/puntseq/master/minimap2/minimap2_k12_genus_april.txt'
## if family:
url = 'https://raw.githubusercontent.com/d-j-k/puntseq/master/minimap2/minimap2_k12_family_april.txt'
s = requests.get(url).content
full1 = pd.read_csv(io.StringIO(s.decode('utf-8')), sep='\t', index_col=0, header=0)

## if genus:
#url = 'https://raw.githubusercontent.com/d-j-k/puntseq/master/minimap2/minimap2_k12_genus_june.txt'
## if family:
url = 'https://raw.githubusercontent.com/d-j-k/puntseq/master/minimap2/minimap2_k12_family_june.txt'
s = requests.get(url).content
full2 = pd.read_csv(io.StringIO(s.decode('utf-8')), sep='\t', index_col=0, header=0)

## if genus:
#url = 'https://raw.githubusercontent.com/d-j-k/puntseq/master/minimap2/minimap2_k12_genus_august.txt'
## if family:
url = 'https://raw.githubusercontent.com/d-j-k/puntseq/master/minimap2/minimap2_k12_family_august.txt'
s = requests.get(url).content
full3 = pd.read_csv(io.StringIO(s.decode('utf-8')), sep='\t', index_col=0, header=0)

## discover samples to exclude (<37k reads)
full1.sum()[full1.sum()<37000]
full2.sum()[full2.sum()<37000]
full3.sum()[full3.sum()<37000]

## list all samples to include (>=37k reads)
## if genus:
...
## if family:
datacolumns_filt = ['1-April_family.tsv',
 '1-August_family.tsv',
 '1-June_family.tsv',
 '2-April_family.tsv',
 '2-August_family.tsv',
 ...
 '7-April_family.tsv',
 '8-April_family.tsv',
 '9.1-August_family.tsv',
 '9.2-August_family.tsv',
 'Mock-April_family.tsv',
 'Mock-August_family.tsv',
 'Mock-June_family.tsv',
 'N-August_family.tsv']
 
posfilt = [np.where(np.array(datacolumns) == x)[0][0] for x in datacolumns_filt]

## save four rarefied datasets for downstream analyses

i = 0
for nr in ['V2','V3','V4','V5']:
    i = i + 1
    dataa = data.copy()
    dataa = dataa[nr]
    ...
    ## if genus
    # dataa.to_csv('final_genus_%i.tsv' %i, sep='\t')
    ## if family
    dataa.to_csv('final_family_%i.tsv' %i, sep='\t')


## final files are available here: https://github.com/d-j-k/puntseq/tree/master/minimap2/downsampled


## also save combined downsampled data from all months together for downstream Mantel test
full1 = full1.transpose()
full2 = full2.transpose()
full3 = full3.transpose()

fullall = full1.append([full2, full3])
fullall = fullall.dropna(how='all')
fullall = fullall.fillna(0)

fullall['month'] = np.repeat(np.array(['April','June','August']),[12,12,12],axis=0)
fullall['targetnice'] = fullall.index

fullall.index = pd.MultiIndex.from_arrays(fullall[['month', 'targetnice']].values.T, names=['month', 'location'])
del fullall['month']; del fullall['targetnice']

## if genus
#fullall.to_csv('All_genus_minimap2.tsv' , sep='\t')
## if family
fullall.to_csv('All_family_minimap2.tsv' , sep='\t')

## the final file is available here: https://github.com/d-j-k/puntseq/tree/master/minimap2/downsampled
...
## choose one file (family or genus level, and rarefaction version)
url = 'https://raw.githubusercontent.com/d-j-k/puntseq/master/minimap2/downsampled/final_family_1.tsv'
s = requests.get(url).content
data = pd.read_csv(io.StringIO(s.decode('utf-8')), sep='\t', index_col=0, header=0)
...
## load combined downsampled dataset
## if genus
# url = 'https://raw.githubusercontent.com/d-j-k/puntseq/master/minimap2/downsampled/All_genus_minimap2.tsv'
## if family
url = 'https://raw.githubusercontent.com/d-j-k/puntseq/master/minimap2/downsampled/All_family_minimap2.tsv'
s = requests.get(url).content
...
## choose one downsampled dataset
nr = 1 

## example for family-level (just exchange "family" for "genus" to run the same analyses on genus level) 
url = 'https://raw.githubusercontent.com/d-j-k/puntseq/master/minimap2/downsampled/final_family_%i.tsv' %nr
s = requests.get(url).content
data = pd.read_csv(io.StringIO(s.decode('utf-8')), sep='\t', index_col=0, header=0)

# plot PCA of all remaining samples (>37k reads)
x = data.copy()

# save positive controls in different dataframes to project them on the PCA later
x2 = x[['Mock-April_family.tsv','Mock-June_family.tsv','Mock-August_family.tsv']]

# delte positive and negative controls
del x['N-August_family.tsv']
del x['Mock-April_family.tsv']; del x['Mock-August_family.tsv']; del x['Mock-June_family.tsv']
...
principalComponents2['target'] = ['Mock-April_family.tsv','Mock-June_family.tsv','Mock-August_family.tsv']
...
fig.savefig('PCA_%i_%i.pdf' %(pc1,pc2), bbox_inches='tight')
...
fig.savefig('PCA_contributions_%s.pdf' %pc_c, bbox_inches='tight')
...
fig.savefig('PCA_ionics_%i_%i.pdf' %(pc1,pc2), bbox_inches='tight')
...
ax.set_xlabel('contribution to %s' %pc_c)
fig.savefig('PCA_ionics_contributions_%s.pdf' %pc_c, bbox_inches='tight')
...
# Drop positive and negative samples
realData = x.drop('N-August_family.tsv')
realData = realData.drop('Mock-August_family.tsv')
realData = realData.drop('Mock-June_family.tsv')
realData = realData.drop('Mock-April_family.tsv')

# Get the proportions of the total amount of bacteria sampled
totalBact = realData.sum()
numBact = sum(totalBact)
bactProportion = totalBact / numBact

# define functions to find outlier bacteria across samples
def getBacteriaPercentagePerLocation(data, bacteria):
    data1 = data.iloc[[0, 1, 2], :]
    data2 = data.iloc[[3, 4], :]
    data3 = data.iloc[[5], :]
    data4 = data.iloc[[6, 7], :]
    data5 = data.iloc[[8, 9], :]
    data6 = data.iloc[[10, 11], :]
    data7 = data.iloc[[12], :]
    data8 = data.iloc[[13], :]
    data9 = data.iloc[[14, 15], :]

    BacteriaPropPerLocation = \
    np.array([data1.loc[:, bacteria].mean(),\
    data2.loc[:, bacteria].mean(),\
    data3.loc[:, bacteria].mean(),\
    data4.loc[:, bacteria].mean(),\
    data5.loc[:, bacteria].mean(),\
    data6.loc[:, bacteria].mean(),\
    data7.loc[:, bacteria].mean(),\
    data8.loc[:, bacteria].mean(),\
    data9.loc[:, bacteria].mean()])

    return BacteriaPropPerLocation

def plotBacteriaPercentagePerLocation(data, bacteria):
    BacteriaPropPerLocation = getBacteriaPercentagePerLocation(data, bacteria)
    plt.bar(range(1, 10), BacteriaPropPerLocation)
    plt.xlabel('Location'); plt.ylabel('reads {}'.format(bacteria))

def plotBacteriaPercentagePerMonth(data, bacteria):
    dataApril = realData.iloc[[0, 3, 5, 6, 8, 12, 13], :]
    dataJune = realData.iloc[[2, 11], :]
    dataAugust = realData.iloc[[1, 4, 7, 9, 10, 14, 15], :]

    BacteriaPropPerMonth = \
    np.array([dataApril.loc[:, bacteria].mean(),\
    dataJune.loc[:, bacteria].mean(),\
    dataAugust.loc[:, bacteria].mean()])

    plt.bar(range(1, 4), BacteriaPropPerMonth)
    plt.xlabel('Month'); plt.ylabel('% of {}'.format(bacteria))

def detectBacteriaOutliers(data, bacteria):
    BacteriaPropPerLocation = getBacteriaPercentagePerLocation(data, bacteria)
    if max(BacteriaPropPerLocation) > 5 * BacteriaPropPerLocation.mean():
        return 1
    else:
        return 0

# examples
totalBact.nlargest(50)
plotBacteriaPercentagePerLocation(realData, 'Deferribacteraceae')
plotBacteriaPercentagePerMonth(realData, 'Burkholderiaceae')

detectBacteriaOutliers(realData, 'Spirosomaceae')

outliers = []
for i in range(0, len(realData.keys())):
    tf = detectBacteriaOutliers(realData, realData.keys()[i])
    if tf:
        outliers += [realData.keys()[i]]

## detect all outlier bacteria that occur in one sample 5x more than the mean across all samples and that have at least 500 reads 
for i in outliers:
    if sum(x.loc[:, i]) > 500:
        print(i)
        plt.figure()
        plotBacteriaPercentagePerLocation(realData, i)
        
## choose bacteria of interest and make nice plot 
fig, ax = plt.subplots(figsize=(8, 8), facecolor='white')
plotBacteriaPercentagePerLocation(realData, 'Bacteriovoracaceae')
