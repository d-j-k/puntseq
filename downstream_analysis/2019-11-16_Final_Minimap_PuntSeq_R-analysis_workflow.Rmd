...
# Summmary
This markdown allows to reproduce the downstream processing performed on classifier outputs. It combines R, Python and Bash code and requires the following packages/dependencies to be installed on your computer before execution:
...
```{r eval=FALSE}
# b. Define global variables and functions
...
outfolder <- str_c(date,"_PuntSeq_Metagenomics-analysis/")

# Basecaller
bcaller <- "Guppy"  #(Guppy, Albacore) If you change bcaller to Albacore manual adjustemnets might be nessesary for some of the code chunks below

# Classifier
classifyr <- "minimap" #(minimap, bracken, bracken16S, centrifuge, centrifuge16S, kraken, kraken16S) If you change classifyr to any other than minimap2, manual adjustemnets will be nessesary for some of the code chunks
...
```
and directories for output data are created.
```{bash eval=FALSE}
...
mkdir $outfolder
mkdir $outfolder/Classifier_output/
```

## Downloading classifier output data
Classifier outputs comprising raw count data can be downloaded from our GitHub directory (https://github.com/d-j-k/puntseq). They contain total count number as well as percentual/normalised count data from all samples taken in April, June and August. Executing the next lines of code will download all nesseary files for this analyisis automatically into the "<OUTFOLDER>/Classifier_output" folder.
```{r eval=FALSE}getwd()

```{r}
# d. Download classifier output data

# download classifier output data for april samples
...
# download classifier output data for june samples
...
# download classifier output data for august samples
...
# Part 1: Quality Control & Downsampling
...
### (to clarify) Summarise information: total data vs. 37,000 reads (No files for some of the samples taken in June?, family.37000 and genus.37000 must be saved together with sampling dataset!)
```{r}
...
### (Please check: AttributeError: 'DataFrame' object has no attribute 'taxRank') Pull single downsampling datasets (and three replicates) for downstream analysis
```{python, eval=FALSE}
<!-- import datetime -->
<!-- import pandas as pd  -->
<!-- import io -->
...
<!-- # choose samples with more than 30k reads in the full dataset to keep for downstream analyses   -->
...
```{r eval=FALSE}
# e. Download downsampled classifier output data
...
for (set in 1:4){
  for (level in c("family","genus")){
    ...
    destfile_downsampling=str_c(outfolder,"Downsampling/Downsampled_",level,"_set",set,"_",classifyr,".tsv")
...
reads.data.raw <- read_tsv("Table_S2.txt")
...
```{r eval=FALSE}
# Load data into dataframe
DNA.data.raw <- read_tsv("Table_S2.txt")
...
# Correlation between DNA yield and PCR yield
 p1 <- ggscatter(DNA.data, x = "DNA_Concentration", y = "X16S_Concentration",
          # add = "reg.line",                         # Add regression line: loss = local regression fitting; reg.line = liniear regression         
          # conf.int = TRUE,                          # Add confidence interval
          # color = "Time", size = 0.3,          # Color by groups "cyl"
...
p2 <- ggscatter(DNA.data, x = "X16S_Concentration", y = "Reads",
          #shape = "Experiment",                             # Change point shape by groups "cyl"
          #facet.by = "Time",
          yscale = "log10",
          #xticks.by = 1
...
Within the next lines of code, raw data is tidied up and prepared for plotting. Full dataset containing all reads obtained as well as downsampled datasets are processed. Datasets were merged into two dataframes containing abosulte, respectively normalised read data of all locations and timepoints. The required steps are individually performed for each dataset.
### Full dataset
Data from April:
```{r eval=FALSE}
# read in family data
...
### Downsampled data
Similar to the full datasets, the set of downsampled data prepared for plotting. 
Create a list of tidied data for the 4 downsampling sets
```{r eval=FALSE}
list1 <- list("","","","")
list2 <- list("","","","")
...
for (set in 1:4){
  ...
  if(level == 1){
    ...
    tidy_classified_reads_downsampled[[level]][[set]] <- gather(classified_reads_downsampled[[level]][[set]], key = "Location", value = "Reads", 2:18)
    ...
    clean_classified_reads_downsampled[[level]][[set]]  # display tidied data table
  }  
}
```

## Plotting data
This section captures all chunks of R code which we used to visualise the data as barchart of total/normalised read counts, barchart of relative abundances, barchart of particular families, heatmap of downsampled data, core microbiom, pathogen analysis...

### Reads per location/time point
To get an overview about the read disribution per sample the following barplot was created:
```{r eval=FALSE}
# Create output directory
system(str_c("mkdir ",outfolder,"Overview-plots/"))
...
Next, the distribution of classified/unclassified reads per location/time point is displayed in four different variants.
```{r eval=FALSE}
...
for (setting in c("samples","samples+controls")){
  if (setting == "samples"){
    subset3_5 <- filter(subset3_4, Location != "N" & Location != "Mock")
  } else {
...
The relative species abundance on the different taxonomic levels was visualised using the following lines of code. The full dataset as well as one downsampled dataset was visualsied in a similar manner, while the latter was restricted to the family level.
#### Full dataset
```{r eval=FALSE}
# create output directory
...
#### Downsampled data
Relative abundance plots on the downsampled data sets where only performed on the family / genus level.
```{r eval=FALSE}
  ...
  for (level in 1:2){
    ...
    for (setting in c("samples","samples+controls")){
      ...
      if (set == 1){
        ...
        subset_rank_info <- subset(subset_rank_top10_2, select = c(Name, Location, Time, Reads)) %>%
        ...
        ggplot(data = subset_rank_3, mapping = aes(x = Time, y = Sum, fill = Name))+
          ...
          annotation_logticks(sides="l")
        # save plot
        ggsave(str_c(outfolder,"Relatve_abundance/Downsampled-dataset/",ranks,"/Set",set,"/Top10-highest-single-abundance/",date,"_Read-numbers-in-comparison_level-",rank_i,"_",ranks,"_",classifyr,"_",setting,".pdf"), width = 60, height = 15, units = "cm")
        ggsave(str_c(outfolder,"Relatve_abundance/Downsampled-dataset/",ranks,"/Set",set,"/Top10-highest-single-abundance/",date,"_Read-numbers-in-comparison_level-",rank_i,"_",ranks,"_",classifyr,"_",setting,".png"), width = 60, height = 15, units = "cm") 
      }
    }
  }
}
...
### Clustering heatmap 
#### (does this make sense?, discuss normalisation - you just apply log10 at the moment, right? I would (afterwards) standardize the features by removing the mean and scaling to unit variance to give each bacteria the same "chance" of showing outliers - does that make sense?) Full dataset
 ```{r eval=FALSE}
# # create output directory
# system(str_c("mkdir ",outfolder,"Heatmap"))
# system(str_c("mkdir ",outfolder,"Heatmap/Full-dataset/"))
# 
# # generate plots for downsampled data on genus and family level
# for (rank in c("genus", "family")){
#     
#   # Where we at?
...
#   if (rank == "genus"){
#     data.tmp <- filter(clean_classified_reads, TaxRank == "G", Percentage != "NA")%>%
#       unite("Colnames", c("Location","Time"), sep = " - ") %>%
#       select(Name, Colnames, Percentage) %>%
#       spread(key = Colnames, value = Percentage)
#       #continue working here
#   }
#   if (rank == "family"){
#     data.37000.tmp <- family.37000
#   
#   
#     ## summarise data from selected downsampled inputs 
#     data.tmp <- sort(unique(c(as.character(data.37000.tmp$`1`$April$sets[,1]),
#                               as.character(data.37000.tmp$`1`$June$sets[,1]),
#                               as.character(data.37000.tmp$`1`$August$sets[,1]),
#                               as.character(data.37000.tmp$`2`$April$sets[,1]),
#                               as.character(data.37000.tmp$`2`$August$sets[,1]),
#                               as.character(data.37000.tmp$`3`$April$sets[,1]),
#                               as.character(data.37000.tmp$`4`$April$sets[,1]),
#                               as.character(data.37000.tmp$`4`$August$sets[,1]),
#                               as.character(data.37000.tmp$`5`$April$sets[,1]),
#                               as.character(data.37000.tmp$`5`$August$sets[,1]),
#                               as.character(data.37000.tmp$`6`$June$sets[,1]),
#                               as.character(data.37000.tmp$`6`$August$sets[,1]),
#                               as.character(data.37000.tmp$`7`$April$sets[,1]),
#                               as.character(data.37000.tmp$`8`$April$sets[,1]),
#                               as.character(data.37000.tmp$`9.1`$August$sets[,1]),
#                               as.character(data.37000.tmp$`9.2`$August$sets[,1]),
#                               as.character(data.37000.tmp$`Mock`$April$sets[,1]),
#                               as.character(data.37000.tmp$`Mock`$June$sets[,1]),
#                               as.character(data.37000.tmp$`Mock`$August$sets[,1]),
#                               as.character(data.37000.tmp$`N`$April$sets[,1]),
#                               as.character(data.37000.tmp$`N`$June$sets[,1]),
#                               as.character(data.37000.tmp$`N`$August$sets[,1])
#                               )))
#   } 
#   
#     # initialise data matrix for heatmap
#     heatmap <- matrix(NA, ncol = 22, nrow = length(data.tmp))
#     # define row names 
#     rownames(heatmap) <- data.tmp
#     # define column names 
#     colnames(heatmap) <- c('1 - April', '1 - June', '1 - August', '2 - April', '2 - August', '3 - April', '4 - April', '4 - August', '5 - April', '5 - August', '6 - June', '6 - August', '7 - April', '8 - April', '9.1 - August', '9.2 - August', 'Mock - April', 'Mock - June', 'Mock - August', 'N - April', 'N - June', 'N - August')
#     
#     # map count data into heatmap matrix 
#     heatmap[,1] <- data.37000.tmp$`1`$April$set[match(rownames(heatmap), data.37000.tmp$`1`$April$sets[,1]),set+2]
#     heatmap[,2] <- data.37000.tmp$`1`$June$set[match(rownames(heatmap), data.37000.tmp$`1`$June$sets[,1]),set+2]
#     heatmap[,3] <- data.37000.tmp$`1`$August$set[match(rownames(heatmap), data.37000.tmp$`1`$August$sets[,1]),set+2]
#     heatmap[,4] <- data.37000.tmp$`2`$April$set[match(rownames(heatmap), data.37000.tmp$`2`$April$sets[,1]),set+2]
#     heatmap[,5] <- data.37000.tmp$`2`$August$set[match(rownames(heatmap), data.37000.tmp$`2`$August$sets[,1]),set+2]
#     heatmap[,6] <- data.37000.tmp$`3`$April$set[match(rownames(heatmap), data.37000.tmp$`3`$April$sets[,1]),set+2]
#     heatmap[,7] <- data.37000.tmp$`4`$April$set[match(rownames(heatmap), data.37000.tmp$`4`$April$sets[,1]),set+2]
#     heatmap[,8] <- data.37000.tmp$`4`$August$set[match(rownames(heatmap), data.37000.tmp$`4`$August$sets[,1]),set+2]
#     heatmap[,9] <- data.37000.tmp$`5`$April$set[match(rownames(heatmap), data.37000.tmp$`5`$April$sets[,1]),set+2]
#     heatmap[,10] <- data.37000.tmp$`5`$August$set[match(rownames(heatmap), data.37000.tmp$`5`$August$sets[,1]),set+2]
#     heatmap[,11] <- data.37000.tmp$`6`$June$set[match(rownames(heatmap), data.37000.tmp$`6`$June$sets[,1]),set+2]
#     heatmap[,12] <- data.37000.tmp$`6`$August$set[match(rownames(heatmap), data.37000.tmp$`6`$August$sets[,1]),set+2]
#     heatmap[,13] <- data.37000.tmp$`7`$April$set[match(rownames(heatmap), data.37000.tmp$`7`$April$sets[,1]),set+2]
#     heatmap[,14] <- data.37000.tmp$`8`$April$set[match(rownames(heatmap), data.37000.tmp$`8`$April$sets[,1]),set+2]
#     heatmap[,15] <- data.37000.tmp$`9.1`$August$set[match(rownames(heatmap), data.37000.tmp$`9.1`$August$sets[,1]),set+2]
#     heatmap[,16] <- data.37000.tmp$`9.2`$August$set[match(rownames(heatmap), data.37000.tmp$`9.2`$August$sets[,1]),set+2]
#     heatmap[,17] <- data.37000.tmp$`Mock`$April$set[match(rownames(heatmap), data.37000.tmp$`Mock`$April$sets[,1]),set+2]
#     heatmap[,18] <- data.37000.tmp$`Mock`$June$set[match(rownames(heatmap), data.37000.tmp$`Mock`$June$sets[,1]),set+2]
#     heatmap[,19] <- data.37000.tmp$`Mock`$August$set[match(rownames(heatmap), data.37000.tmp$`Mock`$August$sets[,1]),set+2]
#     heatmap[,20] <- data.37000.tmp$`N`$April$set[match(rownames(heatmap), data.37000.tmp$`N`$April$sets[,1]),set+2]
#     heatmap[,21] <- data.37000.tmp$`N`$June$set[match(rownames(heatmap), data.37000.tmp$`N`$June$sets[,1]),set+2]
#     heatmap[,22] <- data.37000.tmp$`N`$August$set[match(rownames(heatmap), data.37000.tmp$`N`$August$sets[,1]),set+2]
#     
...
core.microbiome <- filter(core.microbiome, percentage > 1) %>%
arrange(desc(percentage))
  
# add clutster information
load(file.path(str_c(outfolder,"Heatmap/Downsampled-dataset/Set",set,"/",date,"_cluster-mapping_downsampled_set-",set,"_genus-level_samples+controls.RData")))
core.microbiome <- left_join(core.microbiome, cluster.mapping)

labelling <- select(core.microbiome, Name) %>%
  unique()
core.microbiome <- mutate(core.microbiome, Name = factor(Name, levels = rev(labelling$Name)))

p2 <-ggplot(core.microbiome,aes(x=Name,y=percentage)) +
  ...
  scale_y_continuous(name ="Reads",trans=log10_trans(), breaks=c(0.5, 1, 10), label = comma)+
  facet_grid(cols = vars(Time))+ 
  coord_trans(y="log10")+
...
p1<- ggplot(core.microbiome, aes(x=Name, y=0.5, fill=Cluster)) +
...
ggsave(str_c(outfolder,"Core-microbiome/",ranks,"/Set",set,"/Top10-highest-single-abundance/",date,"_Read-numbers-in-comparison_level-",rank_i,"_",ranks,"_",classifyr,"_",setting,".pdf"), width = 60, height = 15, units = "cm")
ggsave(str_c(outfolder,"Relatve_abundance/Downsampled-dataset/",ranks,"/Set",set,"/Top10-highest-single-abundance/",date,"_Read-numbers-in-comparison_level-",rank_i,"_",ranks,"_",classifyr,"_",setting,".png"), width = 60, height = 15, units = "cm") 
